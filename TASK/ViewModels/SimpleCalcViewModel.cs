...
{
	class SimpleCalcViewModel : Conductor<object>
	{
		...
		public SimpleCalcViewModel(ICalculations calculator, IWindowManager manager)
		{
		...
		{
			var result = "0";
			string logString = string.Format("New calculation of unary operator: {0}{1} = ", a, function);

			try
			{
				result = _calculator.Calculate(a, function);

				_log.Info(string.Format(logString, result));
			}
		...
		{
			var result = "0";
			string logString = string.Format("New calculation: {0} {1} {2} = ", a, function, b);

			try
			{
				result = _calculator.Calculate(a, b, function);

				_log.Info(string.Concat(logString, result));
			}
			catch (ArgumentException e)
			{
				_log.Error(e.Message, e);
